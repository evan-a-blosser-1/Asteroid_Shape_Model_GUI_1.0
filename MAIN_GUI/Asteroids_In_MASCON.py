...
class ApplicationWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        #############################
        ################ Window Title
        central_widget = QLabel("Asteroids In MASCON")
        self.setWindowTitle("Asteroids In MASCON") 
        ...
        self.combo.addItems(["Welcome","Apophis","Apophis MASCON III", 
                             "Apophis MASCON VIII", "Arrokoth", "Bilbo",
    ...
    def on_click(self, event):
        azim, elev = self.axis.azim, self.axis.elev
    ...
    def set_canvas_configuration(self, theme='Dark Mode'):
        ...
        self.axis = self.canvas.figure.add_subplot(projection="3d")
        # Axis Labels 
        ...
        self.axis.xaxis.set_pane_color((0.0, 0.0,     
                                    0.0, 0.0))       
        self.axis.yaxis.set_pane_color((0.0, 0.0,     
                                    0.0, 0.0))       
        self.axis.zaxis.set_pane_color((0.0, 0.0,     
                                    0.0, 0.0))       
...
##################
    ...
    def Apophis(self):
        ...
        self.Z = Apophis_CM[:,2]
        # Plot Asteroid
        self.axis.scatter3D(self.X,self.Y,self.Z,
               marker='.',
               color='#D41159')
        ...
        self.axis.add_collection3d(Asteroid_Mesh) 
    ...
    def ApophisM3(self):
        ...
        self.set_canvas_configuration()
        # Read in calculated Center of Masses
        Init_Cond_M3 = np.loadtxt('Asteroid_CM/Apophis_M3CM.dat', delimiter=' ',dtype=float)
        # Assign Asteroid Data
        #################################################
        ###################################### Layer I
        self.axis.scatter3D(Init_Cond_M3[:,0],
                            Init_Cond_M3[:,1],
                            Init_Cond_M3[:,2],
        ...
        #################################################
        ###################################### Layer II
        self.axis.scatter3D(Init_Cond_M3[:,3],
                            Init_Cond_M3[:,4],
                            Init_Cond_M3[:,5],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        #################################################
        ###################################### Layer III
        self.axis.scatter3D(Init_Cond_M3[:,6],
                            Init_Cond_M3[:,7],
                            Init_Cond_M3[:,8],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        # Set Asteroid Aspect Ratio 
        self.axis.set_box_aspect(                                   
            [np.ptp(coord) for coord in [Init_Cond_M3[:,0], 
                                         Init_Cond_M3[:,1],
                                         Init_Cond_M3[:,2]]]) 
        # Draw Asteroid
        self.canvas.draw()
        # Set Asteroid Information out
        with open("Asteroid_Info/ApophisM3.md", "r") as file:
                Output_Message = file.read()
                self.Asteroid_info.setMarkdown(Output_Message)
    ########################################################
    #
    ...
    def ApophisM8(self):
        ...
        self.set_canvas_configuration()
        # Read in calculated Center of Masses
        Init_Cond_M8 = np.loadtxt('Asteroid_CM/Apophis_M8CM.dat', delimiter=' ',dtype=float)
        #################################################
        ##################################### Layer I
        self.axis.scatter3D(Init_Cond_M8[:,0],
                            Init_Cond_M8[:,1],
                            Init_Cond_M8[:,2],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        #################################################
        ##################################### Layer II
        self.axis.scatter3D(Init_Cond_M8[:,3],
                            Init_Cond_M8[:,4],
                            Init_Cond_M8[:,5],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        #################################################
        ##################################### Layer III
        self.axis.scatter3D(Init_Cond_M8[:,6],
                            Init_Cond_M8[:,7],
                            Init_Cond_M8[:,8],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        #################################################
        ##################################### Layer IV
        self.axis.scatter3D(Init_Cond_M8[:,9],
                            Init_Cond_M8[:,10],
                            Init_Cond_M8[:,11],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        #################################################
        ##################################### Layer V
        self.axis.scatter3D(Init_Cond_M8[:,12],
                            Init_Cond_M8[:,13],
                            Init_Cond_M8[:,14],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        #################################################
        ##################################### Layer VI
        self.axis.scatter3D(Init_Cond_M8[:,15],
                            Init_Cond_M8[:,16],
                            Init_Cond_M8[:,17],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        #################################################
        ##################################### Layer VII
        self.axis.scatter3D(Init_Cond_M8[:,18],
                            Init_Cond_M8[:,19],
                            Init_Cond_M8[:,20],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        #################################################
        ##################################### Layer VIII
        self.axis.scatter3D(Init_Cond_M8[:,21],
                            Init_Cond_M8[:,22],
                            Init_Cond_M8[:,23],
                            marker=Cm_plot_mark_typ,
                            s=Cm_plot_p_size,
        ...
        # Set Asteroid Aspect Ratio
        self.axis.set_box_aspect(                                   
            [np.ptp(coord) for coord in [Init_Cond_M8[:,0], 
                                         Init_Cond_M8[:,1],
                                         Init_Cond_M8[:,2]]]) 
        # Draw Asteroid
        self.canvas.draw()
        # Set Asteroid Information out
        with open("Asteroid_Info/ApophisM8.md", "r") as file:
                Output_Message = file.read()
                self.Asteroid_info.setMarkdown(Output_Message)
    ########################################################
    #
...
#%% Slots 
    ...
    def combo_option(self, text):
        if text == "Apophis":
            self.Apophis()
        elif text == "Apophis MASCON III":
            self.ApophisM3()
        elif text == "Apophis MASCON VIII":
            self.ApophisM8()   
        elif text == "Welcome":
            self.draw_blank()
        elif text == "Arrokoth":
    ...
    def rotate_azim(self, value):
        self.axis.view_init(self.axis.elev, value)
        self.fig.set_canvas(self.canvas)
        self.canvas.draw()
    @Slot()
    def rotate_elev(self, value):
        self.axis.view_init(value, self.axis.azim)
        self.fig.set_canvas(self.canvas)
        self.canvas.draw()
